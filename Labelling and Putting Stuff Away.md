...
For the most part, audiovisual materials are processed at the item-level to help facilitate preservation via digitization. In addition to finding aid description and access, metadata for AV items are also entered into Jittberug, an online content management system for Wilson's audiovisual holdings. Jitterbug stores and automatically assign AV item call numbers by collection and format. For more on processing audiovisual materials, visit [Audiovisual Materials](https://github.com/UNC-Libraries/TS-Archival-Procedures-Manual/blob/main/Audiovisual%20Materials.md).

Please note audiovisual materials may also be processed and stored at the box-level (see "Audiovisual box" below). 

Existing labels for shared audiovisual boxes, such as "Videotape box", "Audiocassette box", etc, are stored in the file folder located in TS archival's processing room.
...
For the most part, audiodisc call number pre-fixes are the same across all collecting units at Wilson. Call number pre-fixes are based on format:
