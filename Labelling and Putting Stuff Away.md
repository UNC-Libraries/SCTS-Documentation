...
Below is information on labels and shared box types for archival audiovisual formats found at Wilson Library. 

For the most part, audiovisual materials are processed at the item-level to help facilitate digitization. Item call numbers for audiovisual materials are auto generated by Jitterbug, an online content management system for Wilson's audiovisual holdings. Audiovisual materials may also be processed at the box-level (["Audiovisual box"](#### Audiovisual box)). For more on processing audiovisual materials, visit the [Audiovisual Materials](https://github.com/UNC-Libraries/TS-Archival-Procedures-Manual/blob/main/Audiovisual%20Materials.md).
...
Audiocassettes must have cases before they can be shelved. Audiocassettes frequently arrive in their own containers and, typically, it is fine to use these. New audiocassette cases can be found on level 10, Orange.
...
Audiotapes must have boxes before they can be shelved. Audiotapes frequently arrive in their own containers and typically, it is fine to use these. Select new audiotape cases can be found on level 10, Orange.
...
Audiodiscs must have sleeves, or record jackets, before they can be shelved. Audiodiscs frequently arrive in their own sleeves and typically, it is fine to use these. New audiodisc sleeves can be found on level 9, Yellow.
...
SFC audiodiscs also have a distinct call number pre-fix based on format, while audiodiscs from all other collecting units at Wilson have the call number pre-fix "D" for audiodiscs:

Audiodisc (D-collection number/item number) 
SFC Instantaneous Disc (FD-collection number/item number) 
SFC Transcription Disc (TR-collection number/item number) 
SFC LP Disc (FC-collection number/item number) 
SFC 45-rpm Disc (45-collection number/item number) 
SFC 78-rpm Disc (78-collection number/item number) 
SFC Analog Disc (D-collection number/item number) 
...
Ideally Digital audiotapes (DAT) have cases before they can be shelved. DATs typically arrive in their own cases and it is typically fine to use these. 
...
Ideally MiniDiscs have cases before they can be shelved. MiniDiscs typically arrive in their own cases and it is typically fine to use these. 
...
MiniDisc call number pre-fixes are the same across all collecting units at Wilson:
...
8-track tapes do not require cases before they can be shelved. 

8-track tapes are shelved in shared CD boxes on level 10, Orange. 

Please note that Southern Folklife Collection 8-track tapes have a distinct shared box system, "SFC 8-Track box", while 8-track tapes from all other collecting units at Wilson are stored in the shared box, "8-track box".

8-track tape call number pre-fixes are the same across all collecting units at Wilson:

8-track tape (8T-collection number/item number) 

When possible, create front-facing labels for 8-track tapes and place them on the top left front of the item.
[IMAGE]
...
Ideally Videotapes have cases before they can be shelved. Videotapes typically arrive in their own cases and it is typically fine to use these. Select new videotape cases can be found on level 10, Orange.
